...
class Network:
    ...
    def predict(self, inputs):
        ...
        for i in range(len(self.outputs[len(self.outputs)-1])):
            self.outputs[len(self.outputs)-1][i] = self.end_activator.fn(self.outputs[len(self.outputs)-1][i])
    ...
    def backprop(self, y_c, inputs, lrn_rt):
        ...
        for l in range(len(g)-1, 0, -1):
            for n in range(len(g[l])):
                ...
                if l == len(g)-1:
                    s = self.end_activator.dfn(g[l][n])
                dCdz = dC*s
...
g = Network(2, [], Activator(lambda x: relu(x), lambda x: drelu(x)))
